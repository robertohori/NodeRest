...
var userController = function(pool){
    ...
    var post = function(req,res){
        ...
        pool.getConnection(function(err, connection) {
            ...
            var query = connection.query('INSERT INTO tb_user SET ?', post_t, function(err, resuslt){
                if (!err){


                    console.log(query.sql);
                    post_t.id_user = resuslt.insertId;
                    ...
                    var query2 = connection.query('INSERT INTO tb_logs (error,name_table,query) values ("'+err+'","tb_user","post","'+query+'")', function(err2, resuslt){
                        if(err2){
    ...
    var get = function(req,res){
       ...
       var type = req.headers['type'];
        ...
        pool.getConnection(function(err, connection) {
            ...
            var query = connection.query('SELECT * FROM tb_user where '+query_i , function (err, resuslt) {
                if (!err) {
                   //
                        if( resuslt.length >0){
                    ...
                    var query2 = connection.query('INSERT INTO tb_logs (error,name_table,action,query) values ("' + err + '","tb_user","get","'+query+'")', function (err2, resuslt) {
                        if (err2) {
    ...
    var put = function(req,res) {
            ...
            pool.getConnection(function(err, connection) {
                ...
                var query = connection.query('UPDATE tb_user SET '+query_2+' where '+query_i , function (err, resuslt) {
                    if (!err) {
                        var query = connection.query('SELECT * FROM tb_user where '+query_i , function (err, resuslt) {
                            if (!err) {
                                res.status(201).send({ success: true, message: 'Done', data: resuslt  });
